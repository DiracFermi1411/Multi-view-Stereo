...
def compute_right2left_transformation(R_iw, T_iw, R_jw, T_jw):
    ...
    H_iw = np.vstack((np.column_stack((R_iw,T_iw)),last_row))
    H_wj = np.vstack((np.column_stack((R_jw,T_jw)),last_row))

    #Extract R and T from H
    H_ji = H_iw @ np.linalg.inv(H_wj)
    R_ij = H_ji[0:3,0:3]
    T_ij = H_ji[0:3,-1].reshape(3,1)
...
def compute_rectification_R(T_ij):
    ...
    tmp = np.array([float(T_ij[1]),-float(T_ij[0]),0])
...
def postprocess(
    ...
    """Student Code Starts"""
    last_row = np.array([[0,0,0,1]])
    H = np.vstack((np.column_stack((R_wc,T_wc)),last_row))
    H = np.linalg.inv(H)
    newR = H[0:3,0:3]
    newT = H[0:3,-1]
